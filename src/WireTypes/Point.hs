...
cmpReach = cmpBreakdownPoints (\Points{reach} -> reach :: LinearPoints)

cmpEffort :: (a, Breakdown) -> (a, Breakdown) -> Ordering
cmpEffort = cmpBreakdownPoints (\Points{effort} -> effort :: DifficultyPoints)

cmpTime :: (a, Breakdown) -> (a, Breakdown) -> Ordering
cmpTime = cmpBreakdownPoints (\Points{time} -> time :: TimePoints)

cmpArrival :: (a, Breakdown) -> (a, Breakdown) -> Ordering
cmpArrival = cmpBreakdownPoints (\Points{arrival} -> arrival :: ArrivalPoints)
